...
def craw_sina_finance_articles(collection):
    ...
    #     _cookies=r.cookies#获取cookie
        ...
        for j in li[2:]:
            ...
            f=codecs.open(str('E:/Causal_events/sina_economics_articles_1/'+str(article_title).replace(':','').replace('"','').replace('+','').replace('*','').replace('：','').replace('?','').replace(' ','')+'.txt'),'a','utf-8')#标题去除异常字符
            ...
            if (content!=None):
                article_content=content.text.strip().replace('\t','').replace('\n','').replace(' ','').replace('阅读┊收藏┊转载┊喜欢▼┊打印┊举报','').replace('阅读┊评论┊收藏┊转载┊喜欢▼┊打印┊举报','').replace('免责声明：博主所发内容不构成买卖股票依据。股市有风险，入市需谨慎。新浪财经网站提供此互动平台不代表认可其观点。新浪财经所有博主不提供代客理财等非法业务。有私下进行收费咨询或推销其他产品服务，属于非法个人行为，与新浪财经无关，请各位网友务必不要上当受骗！查看博主原文>>@@title@@@@teacher_name@@：@@title@@0转载喜欢','').replace('\r','')
#                 article_content_list=split_sents(article_content)
            else:
                article_content=''
            mu = threading.Lock()
...
if __name__ == '__main__':
    mongo_con=MongoClient('172.20.65.228', 27017)
